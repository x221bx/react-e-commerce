      ...
      body: JSON.stringify({
        ...
        messages: [

          {
            ...
            content: `
Always reply in the same language the user writes in: if the user writes in Arabic (has Arabic letters) reply in Arabic; otherwise reply in English. Avoid static/boilerplate replies and keep answers concise. Do not invent external products; only reference products returned from our catalog/cards.
...
`,
  ...
  async function sendMessage(userMessage) {
    ...
    if (isToxic(trimmed)) {
      ...
      setMessages((prev) => [
...

        {
          ...
          content:
            "I’m here to help with farming questions, products, or orders. Please keep it friendly and on-topic.",
    ...
    // إذا لم تكن هناك نية لترشيح منتجات، أنهِ بالرد النصي فقط
    const wantsProducts =
      (intent?.should_recommend_product || "").toLowerCase() === "yes" ||
      !!intent?.product_type ||
      !!intent?.search_query ||
      localAlt ||
      wantsAltFromIntent;
    ...
    if (!wantsProducts) {
      setMessages((prev) => {
        ...
        updated[index] = { ...updated[index], content: finalAIText };
        return updated;
      });
      return;
    }
    ...
    if (
      wantsProducts &&
      (!productResult || !productResult.items || productResult.items.length === 0)
    ) {
      const isArabic = /[\u0600-\u06FF]/.test(trimmed);
      const extra = isArabic
        ? "\n\nلم أجد منتجات مناسبة لسؤالك في الكتالوج حالياً. ستتوفر قريباً."
        : "\n\nI didn't find matching products in our catalog right now. They should be available soon.";
