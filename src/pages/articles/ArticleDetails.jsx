...
const ArticleDetails = () => {
  ...
  const { article, loading, notFound } = useArticle(articleSlug);
  ...
  const [  setHelpfulFeedback] = useState(null);
  ...
  const handleHelpfulFeedback = (isHelpful) => {
    setHelpfulFeedback(isHelpful);
    // Save feedback to localStorage for admin tracking
    const feedback = {
    ...
    toast.success(isHelpful ? "Thank you for your feedback!" : "We'll work on improving this article.");
  };
  ...
  // Don't show draft articles to regular users
  if (localizedArticle && localizedArticle.status === 'draft' && !user?.isAdmin) {
    return <ErrorMessage />;
  }

  // Check if article is published (only published articles should be visible to clients)
  if (localizedArticle && localizedArticle.status !== 'published' && !loading) {
    return <ErrorMessage />;
  }
